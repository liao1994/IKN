...
namespace Transportlaget
{
	public class Checksum
	{
		...
		public byte[] calcChecksum (ref byte[] buf, int size)
		{
			byte[] buffer = new byte[size-2];
			long sum = 0;
		    var mybuf = new byte[2];

			Array.Copy(buf, 2, buffer, 0, buffer.Length);
			sum = checksum(buffer);
			mybuf[(int)TransCHKSUM.CHKSUMHIGH] = (byte)((sum >> 8) & 255);
			mybuf[(int)TransCHKSUM.CHKSUMLOW] = (byte)(sum & 255);
		    return mybuf;
		}
